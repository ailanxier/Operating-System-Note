...
+ 用户级线程：
  ...
  + 应用程序可以通过使用线程库设计成**多线程程序**。通常，应用程序从**单线程（主线程）**开始，在该线程中开始运行，在其运行的任何时刻，可以通过调用线程库中的派生例程创建一个在相同进程中运行的**新线程（对等线程）**。
...
+ 但内存已无空闲空间时，就需要从内存中调出一页程序或数据，送入磁盘的对换区（生磁盘）。
...
> 概念较多且杂:man_factory_worker:，整理自 PPT 和《王道》
